...
SOURCE = 'data.riksdagen.se (SFS open data)'
...
_FAMILIES: list[dict] = [{'name': 'Statutes (SFS)', 'tool': 'se_search / se_get_act / se_get_text'}]

_GAPS: list[dict] = [{'id': 'SE-001', 'family': 'Statutes (SFS)', 'missing': 'Riksdagen publishes the consolidated text up to the andrad t.o.m. marker; amendments after that marker are not reflected.', 'fallback': 'Check the andrad t.o.m. marker on the record before relying on the text.'}, {'id': 'SE-002', 'family': 'Statutes (SFS)', 'missing': 'Case law is not covered - this connector serves statutes.', 'fallback': "Use the Swedish courts' own decision databases."}, {'id': 'SE-003', 'family': 'Statutes (SFS)', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
