...
Pulls a tiny static ``se-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/se-runtime.json
...
Opt-out: set ``SE_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "SE_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/se-eli-mcp/releases/latest/download/se-runtime.json.gz",
)
_CACHE_NAME = "se-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "se-eli-mcp-runtime (+https://github.com/matematicsolutions/se-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("SE_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "se-eli"
