...
from se_eli_mcp.coverage import build_coverage
from se_eli_mcp.server import se_coverage


def _coverage():
    fn = getattr(se_coverage, "fn", se_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SE_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("se_coverage" in line for line in lines), (
        "an audit entry was written but it does not name se_coverage"
    )
